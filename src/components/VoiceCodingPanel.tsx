...
import { Microphone, MicrophoneSlash, Waveform, SpeakerHigh, Code, Cursor, Lightning } from '@phosphor-icons/react'
import { motion, AnimatePresence } from 'framer-motion'
import { toast } from 'sonner'
import { CustomVoiceCommandTrainer } from './CustomVoiceCommandTrainer'
...
export function VoiceCodingPanel({ onCodeInsert, onCommand, language = 'javascript' }: VoiceCodingPanelProps) {
  const [customCommands] = useKV<CustomVoiceCommand[]>('custom-voice-commands', [])
  const [isDictationMode, setIsDictationMode] = useState(false)
  const [dictatedText, setDictatedText] = useState('')
  const [recentCommands, setRecentCommands] = useState<Array<{ command: string; time: Date }>>([])

  const executeCustomCommand = useCallback(async (cmd: CustomVoiceCommand) => {
    switch (cmd.actionType) {
      case 'insert-code':
        onCodeInsert(cmd.actionData)
        break
      
      case 'ai-generate':
        try {
          const prompt = spark.llmPrompt`${cmd.actionData}
          
Generate code in ${language} based on the above instruction.
Only return the code, no explanations.`
      ...
      case 'run-command':
        onCommand(cmd.actionData)
        break
      
      case 'custom-script':
        try {
          const scriptFunction = new Function('onCodeInsert', 'language', cmd.actionData)
          scriptFunction(onCodeInsert, language)
        } catch (error) {
          toast.error('Failed to execute custom script')
        }
        break
    }
  }, [onCodeInsert, onCommand, language])
  ...
  const customVoiceCommands: VoiceCommand[] = (customCommands || [])
    .filter(cmd => cmd.isActive)
    .map(cmd => ({
      command: cmd.name,
      description: cmd.description,
      patterns: cmd.phrases.map(phrase => new RegExp(`^${phrase.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i')),
      action: () => executeCustomCommand(cmd),
    }))

  const allCommands = [...customVoiceCommands, ...builtInCommands]

  const voiceState = useVoiceCommands(allCommands)
  ...
  useEffect(() => {
    ...
    if (isDictationMode && voiceState.transcript && !allCommands.some(cmd => 
      cmd.patterns.some(p => p.test(voiceState.transcript.toLowerCase()))
    )) {
      setDictatedText((prev) => prev + ' ' + voiceState.transcript)
    }
  }, [voiceState.interimTranscript, voiceState.transcript, isDictationMode, allCommands])
...
Generate clean, well-formatted code based on this description.`
  ...
  }, [voiceState.transcript, allCommands, trackCommand])
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--card)] border-l border-border">
      <Tabs defaultValue="commands" className="h-full flex flex-col">
        <div className="p-4 border-b border-border">
          ...
          <TabsList className="grid w-full grid-cols-2 mb-3">
            ...
            <TabsTrigger value="custom" className="text-xs">
              ...
              Custom ({customVoiceCommands.length})
            </TabsTrigger>
          </TabsList>
        ...
        <TabsContent value="commands" className="flex-1 mt-0 overflow-hidden">
          <ScrollArea className="h-full p-4">
            <div className="space-y-4">
              ...
              <div>
                <h4 className="text-xs font-semibold mb-2">Built-in Commands</h4>
                <div className="space-y-2">
                  {builtInCommands.slice(0, 15).map((cmd, i) => (
        ...
        <TabsContent value="custom" className="flex-1 mt-0 overflow-hidden">
          <CustomVoiceCommandTrainer
            onCommandExecute={(cmd) => executeCustomCommand(cmd)}
